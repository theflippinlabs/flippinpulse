...
export default async function LotteryPage() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-bold mb-1">🎫 Loterie</h1>
      <p className="text-pulse-mute text-sm mb-4">Achète des tickets, tire au sort quand le round se ferme.</p>

      {!round ? (
        <div className="bg-pulse-card border border-pulse-border rounded-xl p-6 text-center">
          <div className="text-4xl mb-2">🎫</div>
          <div className="font-semibold">Pas de round actif</div>
          <div className="text-sm text-pulse-mute mt-1">Reviens plus tard, un Lord peut en démarrer un.</div>
        </div>
      ) : (
        <>
          <div className="bg-gradient-to-br from-pulse-gold/25 to-pulse-gold/5 border border-pulse-gold/40 rounded-2xl p-5 mb-4 text-center relative overflow-hidden">
            ...
            <div className="relative">
              <div className="text-xs uppercase text-pulse-mute tracking-wide">Cagnotte</div>
              <div className="text-5xl font-bold text-pulse-gold my-2">{fmt(round.pot_pulse)}</div>
              <div className="text-xs text-pulse-mute">PULSE · {round.total_tickets} ticket{round.total_tickets > 1 ? 's' : ''} vendu{round.total_tickets > 1 ? 's' : ''}</div>
              {round.draw_at && (
                <div className="text-xs text-pulse-mute mt-2">Tirage : {new Date(round.draw_at).toLocaleString()}</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-pulse-card border border-pulse-border rounded-xl p-4 mb-4 grid grid-cols-2 gap-3">
            <div>
              <div className="text-xs uppercase text-pulse-mute">Prix / ticket</div>
              <div className="text-xl font-bold text-pulse-gold mt-1">{fmt(round.ticket_price)} PULSE</div>
            </div>
            <div>
              <div className="text-xs uppercase text-pulse-mute">Tes tickets</div>
