...
export default async function GiveawaysPage() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-bold mb-1">🎉 {t('giveaways.title')}</h1>
      <p className="text-pulse-mute text-sm mb-4">{t('giveaways.subtitle')}</p>

      {!active.length && (
        <div className="bg-pulse-card border border-pulse-border rounded-xl p-6 text-center mb-6">
          <div className="text-4xl mb-2">🎉</div>
          <div className="font-semibold">{t('giveaways.empty_title')}</div>
          <div className="text-sm text-pulse-mute mt-1">{t('giveaways.empty_hint')}</div>
        </div>
      )}

      {active.length > 0 && (
        <section className="mb-6">
          <h2 className="text-xs uppercase text-pulse-mute tracking-wide mb-2">{t('giveaways.running')}</h2>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-pulse-mute mt-0.5">
                      {g.winners_count} {g.winners_count > 1 ? t('common.winners') : t('common.winner')}
                      {g.prize_pulse ? ` · ${fmt(g.prize_pulse)} PULSE ${t('common.each')}` : ''}
                    </div>
                    <div className="text-xs text-pulse-mute mt-1">
                      {t('giveaways.end')} : {new Date(g.end_at).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
      ...
      {ended.length > 0 && (
        <section>
          <h2 className="text-xs uppercase text-pulse-mute tracking-wide mb-2">{t('giveaways.ended')}</h2>
