...
export function startPulsar(client: Client, intervalMs = 5 * 60_000): void {
  interval = setInterval(() => {
    for (const guild of client.guilds.cache.values()) {
      tickGuild(client, guild.id).catch(err => log('ERROR', 'Pulsar scheduler tick failed', err));
    }
  }, intervalMs);
}
...
// Manual trigger (used by the admin panel "post now" button). Must run inside a guild context.
