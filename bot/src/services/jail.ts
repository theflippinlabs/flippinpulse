...
export function parseJailDuration(input: string | null | undefined): JailDuration | null {
  ...
  const secondsPer: Record<string, number> = {
    ...
    mo: 2_592_000, mon: 2_592_000, month: 2_592_000, months: 2_592_000, mth: 2_592_000,
    y: 31_536_000, yr: 31_536_000, year: 31_536_000, years: 31_536_000,
  };
  ...
  const tick = async () => {
    try {
      ...
      for (const j of expired) {
        ...
        if (!guild) continue;
        await runWithGuild(j.guild_id, async () => {
          const cfg = getJailConfig();
          if (!cfg.role_id) return;
