...
// ---- Active objective cache, per guild (keeps per-message tracking cheap) ----
const objCache = new Map<string, { list: Challenge[]; at: number }>();

async function activeObjectives(guildId: string): Promise<Challenge[]> {
  const cached = objCache.get(guildId);
  if (cached && Date.now() - cached.at < 30_000) return cached.list;
  const { data } = await supabase.from('pulse_challenges').select('*')
    .eq('guild_id', guildId).eq('status', 'active').in('kind', ['daily', 'weekly']);
  const list = (data ?? []) as Challenge[];
  objCache.set(guildId, { list, at: Date.now() });
  return list;
}
function invalidateCache(guildId: string) { objCache.delete(guildId); }

// ---- Progress tracking for objective missions (call within guild context) ----
...
export async function recordChallengeMetric(client: Client | null, discordId: string, username: string | null, metric: Metric, amount = 1): Promise<void> {
  ...
  const objs = (await activeObjectives(guildId)).filter(c => c.metric === metric);
  ...
  for (const c of objs) {
    try {
      ...
      await supabase.from('challenge_claims').upsert({
        challenge_id: c.id, guild_id: guildId, discord_id: discordId, progress, completed, updated_at: new Date().toISOString(),
      }, { onConflict: 'challenge_id,discord_id' });
      if (completed) {
        let name = username;
        if (!name) {
          const { data: u } = await supabase.from('discord_users').select('username').eq('guild_id', guildId).eq('discord_id', discordId).maybeSingle();
...
// ---- Launching (call within guild context) ----
export async function launchFlash(client: Client, channelId: string, opts: { reward: number; maxWinners: number; durationMin: number }): Promise<boolean> {
  ...
  const { data: row, error } = await supabase.from('pulse_challenges').insert({
    guild_id: guildId, kind: 'flash', title, description, reward: opts.reward, max_winners: opts.maxWinners,
...
export async function launchRiddle(client: Client, channelId: string, opts: { reward: number; durationMin: number }): Promise<boolean> {
  ...
  const expires = new Date(Date.now() + opts.durationMin * 60_000).toISOString();
  const { data: row, error } = await supabase.from('pulse_challenges').insert({
    guild_id: guildId, kind: 'riddle', title: '🧩 Riddle Mission', description: r.riddle, answer: norm(r.answer),
...
export async function launchObjective(client: Client, channelId: string, opts: { kind: 'daily' | 'weekly'; metric: Metric; goal: number; reward: number }): Promise<boolean> {
  ...
  const { error } = await supabase.from('pulse_challenges').insert({
    guild_id: guildId, kind: opts.kind, title, description, metric: opts.metric, goal: opts.goal, reward: opts.reward,
    channel_id: channelId, status: 'active', expires_at: expires,
  });
...
// ---- Interaction handling (call within guild context) ----
export async function handleChallengeInteraction(interaction: ButtonInteraction | ModalSubmitInteraction): Promise<void> {
  ...
  if (interaction.isButton() && action === 'claim') {
    ...
    await supabase.from('challenge_claims').insert({ challenge_id: ch.id, guild_id: ch.guild_id, discord_id: interaction.user.id, completed: true });
...
// ---- Admin / panel helpers (call within guild context) ----
export async function listActiveChallenges(): Promise<Challenge[]> {
  const { data } = await supabase.from('pulse_challenges').select('*')
    .eq('guild_id', currentGuildId()).eq('status', 'active').order('created_at', { ascending: false }).limit(15);
...
// ---- Scheduler: expiry (global) + per-guild autonomous launches ----
...
async function tick(client: Client): Promise<void> {
  // Expire due missions across all guilds.
