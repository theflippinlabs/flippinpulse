...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (sub === 'generate') {
    ...
    // Skip questions whose exact text already exists.
    const texts = generated.map(q => q.question);
    const { data: existing } = await supabase.from('quiz_questions').select('question').in('question', texts);
    const existingSet = new Set((existing ?? []).map(r => r.question));

    const toInsert = generated
      .filter(q => !existingSet.has(q.question))
      .map(q => ({ question: q.question, choices_json: q.choices, correct_index: q.correct_index, category, is_active: true }));
